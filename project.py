    ...
    while True:
        ...
        for i in range(3):
            try:
                ...
                print(f"familu id:{faimly_name}")
                sort_c=textcase.sort_children_num()
                print(f"id of sort by childern: {sort_c}")
                withoutc=textcase.without_children()
                print(f"the id witou child {withoutc}")
                singlep=textcase.single_parents_child()
                print(f"the id have only one parent {singlep}")
                odd=textcase.odd_age()
                print(f"the id age id odd {odd}")
                print(f"the parents number check reuslt: {textcase.check_parents_num()}")
            except ValueError:                
