...
class function:
    ...
    def most_dead_year(self):
        year=dict()
        for i in self.indi.values():
            if i.birthday.year in year.keys():
                year[i.birthday.year]+=1
            else:
                year.setdefault(i.birthday.year,1)
        return int(sorted(year.items(), key = lambda kv:(kv[1], kv[0]),reverse=True)[0][0])
    
    def Adult_unmarried(self):
        li=list()
        for i in self.indi.values():
            if i.age>=18 and i.married==False:
                li.append(i)
        return li
    ...
    def chil_older_p(self):
        li=list()
        for i in self.fami.values():
            age=list()
            for l in i.children:
                age.append(self.indi[l].age)
            cmax_age=max(age)
            pmax_age=max(self.indi[i.wifeId].age,self.indi[i.husbandId].age)
            if cmax_age>pmax_age:
                li.append(i)
        return li

    def chil_dead_bf_p(self):
        li=list()
        for i in self.fami.values():
            age=list()
            for l in i.children:
                if(self.indi[l].death!="NA"):
                    age.append(self.indi[l].death.year)
            cd_age=min(age)
            if(self.indi[i.wifeId].death=="NA" and self.indi[i.husbandId].death=="NA"):
                pd_age=min(self.indi[i.wifeId].death.year,self.indi[i.husbandId].death.year)
            elif(self.indi[i.wifeId].death=="NA"):
                pd_age=self.indi[i.husbandId].death.year
            elif(self.indi[i.husbandId].death=="NA"):
                pd_age=self.indi[i.wifeId].death.year
            else:
                li.append(i)
                continue
            if cd_age<pd_age:
    ...
    while True:
        ...
        for i in range(3):
            try:
                ...
                textcase.PPrint()
                for i in textcase.Adult_unmarried():
                    print(i.name,end=' ')
                print()
                for i in textcase.age_10_cp():
                    print(i.id,end=' ')
                print()
                
                print(textcase.most_dead_year())
                
                for i in textcase.same_gender_cp():
                    print(i.id,end=' ')
                print()
                for i in textcase.chil_older_p():
                    print(i.id,end=' ')
                print()
                for i in textcase.chil_dead_bf_p():
                    print(i.id,end=' ')
                print()
            except ValueError:                
